...
function scene:create()
	...
	for i = -6, 5 do
		...
		text:setFillColor(0.7,0.2,0.3)

		character.x, character.y = i * 72, 0
		transition.from(character, {time = 500, delay = 100 * j, y = _H + 100, transition = easing.inOutBack })
		j = j + 1
	end
		...
		onRelease = function()
			local options = {
             effect = "zoomInOutFadeRotate",
              time = 500,
              params = { level="scenes.level_select", score=currentScore }
}
	...
	transition.to(self.playButton, {time = 1200, delay = 500, y = _H - 128 - self.playButton.height / 2, transition = easing.inOutQuint, onComplete = function(object1)
		transition.to(object1, {time = 800, x = _W - 64 - self.playButton.width / 2, transition = easing.inOutQuint , onComplete = function(object2)
...
function scene:gotoPreviousScene()
	native.showAlert('Cannon Shoot', 'Tem certeza de que deseja sair desse nível?', {'Sim', 'Nao'}, function(event)
